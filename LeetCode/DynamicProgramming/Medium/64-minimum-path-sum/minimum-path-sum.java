// class Solution { //using extra 2d array
//     public int minPathSum(int[][] grid) {
//         int m = grid.length;
//         int n = grid[0].length;
//         int[][] dp = new int[m][n];
//         for(int i = 0 ; i <m;i++){
//             for(int j = 0 ; j<n;j++){
//                 if(i==0 && j==0) dp[0][0] = grid[0][0];
//                 else if(i==0) dp[i][j] = dp[i][j-1] + grid[i][j];
//                 else if(j==0) dp[i][j] = dp[i-1][j] + grid[i][j];
//                 else dp[i][j] = grid[i][j] + Math.min(dp[i-1][j],dp[i][j-1]);
//             }
//         }
//         return dp[m-1][n-1];
//     }
// }

// class Solution { // Without using extra 2d array
//     public int minPathSum(int[][] grid) {
//         int m = grid.length;
//         int n = grid[0].length;
//         for(int i = 0 ; i <m;i++){
//             for(int j = 0 ; j<n;j++){
//                 if(i==0 && j==0) continue;
//                 else if(i==0) grid[i][j] += grid[i][j-1];
//                 else if(j==0) grid[i][j] += grid[i-1][j];
//                 else grid[i][j] += Math.min(grid[i-1][j],grid[i][j-1]);
//             }
//         }
//         return grid[m-1][n-1];
//     }
// } 


class Solution {  // top - down
    public int minPathSum(int[][] grid) {
        int m = grid.length;
        int n = grid[0].length;
        int[][] dp = new int[m][n];
        for(int i = 0 ; i <m;i++){
            for(int j = 0 ; j<n;j++){
                dp[i][j]=-1;
            }
        }
        return helper(0,0,m,n,grid,dp);
    }
    public int helper(int i,int j,Integer m,Integer n,int[][] grid,int[][] dp){
        if(i==m-1 && j==n-1) return grid[i][j];
        if(i>m-1 || j>n-1) return Integer.MAX_VALUE;
        if(dp[i][j]!=-1) return dp[i][j];
        return dp[i][j] = grid[i][j] + Math.min(helper(i+1,j,m,n,grid,dp),helper(i,j+1,m,n,grid,dp));
    }
}